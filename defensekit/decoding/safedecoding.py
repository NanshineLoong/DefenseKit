...
SafeDecoding
============
A Decoding method that ensure the safety of the generated text.
...
def get_inputs(prompt, tokenizer, return_token_type_ids=True):
    """
    Get the inputs for the model.

    :param str prompt: The prompt.
    :param Any tokenizer: The tokenizer.
    :return dict: The inputs.
    """
...
def safedecoding(query, model, tokenizer, **kwargs):
    ...
    gen_config.max_new_tokens = max_token_len
    gen_config.do_sample = do_sample
    gen_config.top_p = top_p


    # ---- adapted from safedecoding_lora  ---- #
    ...
    while step <= min(max_token_len, first_m):  # Loop until we reach the first m tokens
        # Generate the next token
        # duplicate inputs for two original and expert model
        inputs_duplicated = {k:v.repeat(2,1) for k,v in inputs.items()}

        outputs = model.generate(**inputs_duplicated,
        ...
        if verbose:
            logging.info(f"Selected token: {tokenizer.decode(selected_token_id.item())}, ID: {selected_token_id.item()}")
        ...
        inputs['attention_mask'] = torch.cat([inputs['attention_mask'], torch.tensor([[1]], device=model.device)], dim=1)
